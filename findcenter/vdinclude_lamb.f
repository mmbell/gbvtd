...
*****************************************************************
	...
	real vtcoef(maxcoeffs),aa(maxcoeffs)
        real vtc1_out,vts1_out,vrc1_out,vrs1_out
        real vtc2_out,vts2_out,vrc2_out,vrs2_out
        real vtc3_out,vts3_out,vrc3_out,vrs3_out
	pi=acos(-1.)
...
c	  vtcoef(1)=-B1-B3                            !VTC0
c	  vtcoef(4)=A2-A0+A4+(A0+A2+A4)*cp            !VTS1
c	  vtcoef(3)=-2.*(B2+B4)                       !VTC1
c	  vtcoef(5)=-2.*B3                            !VTC2
c	  vtcoef(6)=2.*A3                             !VTS2
c          vtcoef(7)=-2.*B4                            !VTC3
c          vtcoef(8)=2.*A4                             !VTS3
c Lamb formulation
          vtc1_out= -B2
          vts1_out= (A2*(1.+cp)+A0*(cp-1.))/(1.+cp)
          vrc1_out= vts1_out
          vrs1_out= -vtc1_out
          vrc2_out = A3
          vrs2_out = B3
	  ...
	  if (sp.ge.0.99)then
        ...
        vt0=vtcoef(1)
c        if (vt0.lt.vt1.or.vt0.lt.vt2.or.vt0.lt.vt3) then
c           numcoeff=1
c           print *,'VT Error! ',vtcoef
c        endif
