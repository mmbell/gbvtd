...
*****************************************************************
     ...
     X                    flag1,flagwrite)
       ...
       INCLUDE 'gbvtd_parameter.f'
        ...
        real amvt,a4,b4
        integer ip,numcoeff,flag1,flagwrite,i
	real vtcoef(maxcoeffs),aa(maxcoeffs)
        real vorticity,ar,meanflow,meanangle,phasevt1,ampvt1
        real phasevt2,ampvt2,phasevt3,ampvt3,phasevr1,ampvr1
        real phasevr2,ampvr2,phasevr3,ampvr3,vrmean,alpha,wave
        real power,rotate,wave2magn,vrs2,vrc2,dtor
        real vt1,vt2,vt3,vt0
	...
	pi=acos(-1.)
	  ...
	  B3=aa(6)
        ...
        write(17,728) ip,a3,b3,b1
 728    format(i3,1x,3f10.2)
...
c Adjust coefficients based on observed wavenumber 2 in reflectivity
c Used only for analytic Lamb based TCs - MB
c           if (ip.le.ar) then
c              wave2magn=vorticity*alpha/2
c              a3=a3+wave2magn*sin(2*rotate*dtor)/2
c              b3=b3+wave2magn*cos(2*rotate*dtor)/2
c           endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCcccCCcc
	...
	REAL dummy,DVD(NN),phi(NN),sd,pi,xc,yc,theta_t
        real A(NN),X(NN),C(maxcoeffs),theta,scale
...
*************************************************************************
       ...
       pi=acos(-1.)
c       do while (abs(angle-pi).gt. pi)
c          if(angle.gt.0.) then
c             angle=angle-2.*pi
c          else
c             angle=angle+2.*pi
c          endif
c       enddo
c fixed bug 18 Aug 2000, program would not set angle to 0.0
C if angle = 2 * pi
C p dodge 
c fixed bug 11 Dec 2000, program precision problem
c P Harasti

       if (abs(angle).lt.1.0e-06) angle=0.
